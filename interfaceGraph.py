...
def display(ecran, l, h, taille):
    police = 'police/ArcadeAlternate.ttf'
    point1 = 5
    point2 = 2
    ...
    ecran.blit(logo, (LONGUEUR * 0.352, HAUTEUR * 0.75))

    gameIcon = pygame.image.load('image/icon.png')
    pygame.display.set_icon(gameIcon)

    case = pygame.image.load("image/case.png")  # On affiche le tableau
    defaultValue = (l * 0.5) - (44 * (
            taille / 2))  # On place le tableau à la moitié de la fenêtre, puis on soustrait la taille de la moitié des cases pour avoir le même nombre de cases de chaque côté
    absc = defaultValue
    ordo = h * 0.1
    for y in range(taille):
        for x in range(taille):
            value = pygame.font.Font(police, 20).render(".", True, BLACK, WHITE)
